...
const ExportColorModal = ({ colorsPalette, generateColorFor }: ExportColorModalProps) => {
   ...
   const generateColorValues = () => {
      ...
      colorsPalette
         ...
         .map((color: string, index: number) => {
            const name = generateColorFor === 'palette' ? colord(color).toName({ closest: true }) : colorName
            if (colorFormat === 'css')
   ...
   return (
      <div>
         <input type="checkbox" id="export-color-modal" className="modal-toggle" />
         <label htmlFor="export-color-modal" className="modal zoomIn cursor-pointer">
            <label
               ...
               <main className="px-4 pb-3">
                  <InputGroup className="my-4 flex gap-2" minWidth={150}>
                     <Button
                        text="Copy Code"
                        onClick={handleCopy}
                  ...
                  {colorFormat === 'figma' ? (
                     <div className="text-fs-0  overflow-auto" style={{ height: 'calc(100vh - 300px)' }}>
